// shared/schemas/conversations.ts
...
// must be one of the four known backends and `model` must belong to that
// provider's namespace (anthropic/openai use a static enum; opencode and
// github-copilot are prefix-checked/dynamic since their catalogs are owned
// upstream — see `shared/providers/models.ts`).
...
export const CreateConversationBodySchema = z
  .object({
    ...
    provider: z.enum(['anthropic', 'openai', 'opencode', 'github-copilot']),
    // Provider-specific model identifier (e.g. 'opus', 'gpt-5.5',
    // 'opencode/kimi-k2.6', 'gpt-4o'). Always explicit.
